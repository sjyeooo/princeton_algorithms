...
public class Point implements Comparable<Point> {
    ...
    public double slopeTo(Point that) {
        if (this.x == that.x) {
            if (this.y == that.y) { // Points are the same
    ...
    public static void main(String[] args) {
        ...
        assert (p1.slopeOrder().compare(p4, p3) == -1);
        assert (p1.slopeOrder().compare(p4, p2) == -1);
        assert (p1.slopeOrder().compare(p3, p4) == 1);
        assert (p1.slopeOrder().compare(p2, p4) == 1);
        assert (p1.slopeOrder().compare(p2, p5) == 0);  // Same slope
    }
}
